...
const ViewProfile = ({view}) => {
  ...
  const handleClickOpen = () => {
    // setOpen(true);
    axios.delete(`http://127.0.0.1:8080/Following/delete/${Cookies.get('email')}/${view}`).then(response => {
  ...
  const handleClickOpen1 = () => {
    const data = {
        followl : Cookies.get("email"),
        followr : view,
    }
    // setOpen(true);
    axios.post(`http://127.0.0.1:8080/Followers/add`, data).then(response => {
  ...
  useEffect(() => {
    const unSubscribe = onAuthStateChanged(getAuth(), user => {
      ...
      axios.get(`http://127.0.0.1:8080/profile/get/${view}`)
        .then(response => {
     ...
     axios.get(`http://127.0.0.1:8080/Following1/get/${Cookies.get('email')}/${view}`).then(response1 => {
    ...
    if(dataArray.length !== 0){
     ...
     axios.get(`http://127.0.0.1:8080/Feed/get2/${view}`)
        .then(response => {
  ...
  const handleSubmit = async(event) => {
    ...
    return (
      ...
      <div style={{display: 'block', marginRight: '10%'}}>
        ...
        {following > 0 ? <Button variant="outlined" onClick={handleClickOpen}>Following</Button> : <Button variant="contained" onClick={handleClickOpen1}>Follow</Button>}
        <BootstrapDialog
        onClose={handleClose}
