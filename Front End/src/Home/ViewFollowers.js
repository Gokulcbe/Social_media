...
const ViewFollowers = ({view}) => {
    ...
    const handleClickOpen = () => {
      ...
      const handleDelete = (fid)=> {
        axios.delete(`http://127.0.0.1:8080/Following/delete/${fid}`).then(response => {console.log("delete:", response.data);
      ...
      useEffect(() => {
        const unSubscribe = onAuthStateChanged(getAuth(), user => {
          axios.get(`http://127.0.0.1:8080/Following/get/${view}`)
            .then(response => {
            ...
            axios.get(`http://127.0.0.1:8080/Follower/get/${view}`)
            .then((response1) => {
    ...
    return (
        <div>
            <Button><Typography style={{fontSize: '13px'}}>{data2} Posts</Typography></Button>
